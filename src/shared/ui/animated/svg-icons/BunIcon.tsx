...
const BUN_COLORS = {
  body: {
    from: [0.88, 0.12, 85] as OklchTuple,  // Yellow
    to: [0.95, 0.08, 90] as OklchTuple,    // Light cream
  },
  outline: {
    from: [0.3, 0.1, 110] as OklchTuple,   // Dark olive
    to: [0.1, 0.15, 99] as OklchTuple,     // Very dark
  },
};
...
export const BunIcon = observer(function BunIcon({
  className,
  size = '1em',
  bounce = false,
}: BunIconProps) {
  const bodyRef = useRef<IconSpring | null>(null);
  const outlineRef = useRef<IconSpring | null>(null);
  const bounceRef = useRef<NodeJS.Timeout | null>(null);

  // Initialize controllers
  if (!bodyRef.current) {
    bodyRef.current = new IconSpring(
      { fill: BUN_COLORS.body.from },
      themeStore.springConfig
    );
    outlineRef.current = new IconSpring(
      { stroke: BUN_COLORS.outline.from, strokeWidth: 0.75 },
      themeStore.springConfig
    );
  }
  ...
  useEffect(() => {
    if (bounce) {
      const animate = async () => {
        // Animate colors and scale
        await Promise.all([
          body.animateTo({ fill: BUN_COLORS.body.to, scale: 1.1 }),
          outline.strokeTo(BUN_COLORS.outline.to),
        ]);
        await Promise.all([
          body.animateTo({ fill: BUN_COLORS.body.from, scale: 1 }),
          outline.strokeTo(BUN_COLORS.outline.from),
        ]);
      };
      ...
      const loop = () => {
        animate().then(() => {
          if (bounce) {
            bounceRef.current = setTimeout(loop, 100);
          }
        });
      };
